...
# Writable SQLite app-state store shared with the dashboard (same DB the
# dashboard's appState.ts uses). The dashboard reads/writes user-edited overrides
# here; this pipeline reads the review overlay + account_metrics from it and
# writes its acquisition output back to it. Keyed by the legacy filename.
APP_STATE_DB = os.environ.get(
    "APP_STATE_DB", "/root/s3reporting/.cache/prepared/app_state.db"
)
WAREHOUSE_DUCK_DB = os.environ.get(
    "WAREHOUSE_DUCK_DB", "/root/s3reporting/.cache/prepared/warehouse_duck.db"
)
DUCKDB_BIN = os.environ.get("DUCKDB_BIN", "/usr/local/bin/duckdb")
...
# === App-state SQLite store (shared with the dashboard) ===

def app_state_read(key):
    """Read a JSON blob from app_state.db by key (legacy filename). Returns the
    parsed value, or None if the key is absent or the DB doesn't exist yet."""
    if not os.path.exists(APP_STATE_DB):
        log.warning("app_state.db not found at %s", APP_STATE_DB)
        return None
    try:
        conn = sqlite3.connect(f"file:{APP_STATE_DB}?mode=ro", uri=True, timeout=10)
        try:
            row = conn.execute(
        ...
        log.warning("app_state read of %s failed: %s", key, e)
        return None
    if not row:
        return None
    try:
        ...
        log.warning("app_state value for %s is invalid JSON: %s", key, e)
        return None


def app_state_write(key, obj):
    """Upsert a JSON-serialisable value into app_state.db under key. Creates the
    table if absent (matches the dashboard's appState.ts schema)."""
    ...
    try:
        ...
        conn.close()
    log.info("Wrote %s to app_state.db", key)


def load_account_metrics_sqlite():
    """Read the dashboard-published account_metrics from app_state.db and index
    by account_id. The dashboard writes it there from DuckDB (its exact assembly,
    so no drift). Returns {} if absent — enrichment is then skipped (non-fatal)."""
    rows = app_state_read("account_metrics.json")
    if not rows:
        log.warning("account_metrics.json not in app_state.db — running without "
    ...
    log.info("Loaded account_metrics from app_state.db: %d accounts.", len(indexed))
    return indexed


def load_review_overlay_sqlite():
    """Read CS review decisions + exclusions from app_state.db (the dashboard
    writes them there). Both optional — missing keys mean no overlay applied."""
    decisions = app_state_read("mailshake_match_decisions.json") or []
    exclusions = app_state_read("mailshake_exclusions.json") or []
    if not isinstance(decisions, list):
        log.warning("mailshake_match_decisions is not a list — ignoring.")
        decisions = []
    if not isinstance(exclusions, list):
        log.warning("mailshake_exclusions is not a list — ignoring.")
        exclusions = []
    log.info("Review overlay (SQLite): %d decisions, %d exclusions.",
             len(decisions), len(exclusions))
...
def run(dry_run=False, local_dir=None):
    ...
    # 1. Pull all campaigns and export their recipients.
    ...
    # 2. Load TryBooking Users + Accounts from S3.
    log.info("Loading Users and Accounts from S3...")
    users_df = load_users()
    accounts_df = load_accounts()
    by_email, by_domain = build_owners_lookup(users_df, accounts_df)

    # 3. Load account metrics + CS review overlay from the shared SQLite store
    # (app_state.db). The dashboard publishes account_metrics there from DuckDB
    # and writes decisions/exclusions there directly. Both non-fatal if absent.
    metrics_by_account = load_account_metrics_sqlite()
    decisions, exclusions = load_review_overlay_sqlite()
    ...
    # 5. Optionally save the CSV + run_state locally (for debugging); always
    #    publish the acquisition report + run_state to the shared SQLite store,
    #    which is what the dashboard reads.
    if local_dir:
        os.makedirs(local_dir, exist_ok=True)
        local_files = {
            "mailshake_acquisition.json": json.dumps(
                records, ensure_ascii=False, separators=(",", ":")).encode("utf-8"),
            "mailshake_acquisition.csv": records_to_csv_bytes(records),
            "mailshake_run_state.json": json.dumps(
                run_state, ensure_ascii=False, indent=2).encode("utf-8"),
        }
    ...
    if dry_run:
        ...
        app_state_write("mailshake_acquisition.json", records)
        app_state_write("mailshake_run_state.json", run_state)
